...
async function loadCommonJs(path, dependencies = {}) {
  ...
  vm.runInNewContext(output, { module: testModule, exports: testModule.exports, require: (id) => dependencies[id], Set, Math, String, RegExp });
...
test("cada ficha aprobada mantiene diez variantes trazables", () => {
  assert.equal(catalog.knowledgeCatalog.length, 7);
  assert.equal(catalog.YOL1_KNOWLEDGE_VERSION, "lab-kb-2026-08-26.3");
...
test("el conocimiento aprobado usa el mismo cierre de pendientes que la interfaz", () => {
  ...
  for (const card of pendingCards) {
    assert.match(card.expectedAnswer.next, /Abre Cuentas Claras/i);
    assert.match(card.expectedAnswer.next, /revisar el pendiente exacto|ver el acuerdo/i);
