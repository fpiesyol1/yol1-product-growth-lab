...
check("migración de deployment fail-closed declarada", packageJson.scripts?.["db:migrate:deployment"] === "node scripts/migrate-debt-center-deployment.mjs", "El script db:migrate:deployment debe usar su guardrail dedicado.");

const deploymentMigration = read("scripts/migrate-debt-center-deployment.mjs");
const vercelConfiguration = read("vercel.json");
check("deployment migra antes del build", vercelConfiguration.includes("pnpm run db:migrate:deployment && pnpm run build"), "vercel.json debe ejecutar la migración protegida antes del build.");
check("migración limitada por entorno, target, rama, plan y Neon", deploymentMigration.includes('environment !== "preview" && environment !== "production"') && deploymentMigration.includes('targetEnvironment !== "preview"') && deploymentMigration.includes('targetEnvironment !== "production"') && deploymentMigration.includes('ALLOW_PREVIEW_DB_MIGRATIONS !== "true"') && deploymentMigration.includes('ALLOW_PRODUCTION_DB_MIGRATIONS !== "true"') && deploymentMigration.includes('PRODUCTION_DB_MIGRATION_PLAN !== "debt-center-0000-0001"') && deploymentMigration.includes('.endsWith(".neon.tech")') && deploymentMigration.includes('gitRef !== "main"'), "La migración debe fallar cerrada sin entorno, target, rama, plan, opt-in o endpoint Neon correctos.");
...
check("contrato de entorno documentado", ["DATABASE_URL=", "NEXT_PUBLIC_SITE_URL=", "DEBT_CENTER_SIMULATOR_ENABLED=false", "ALLOW_PREVIEW_DB_MIGRATIONS=false", "ALLOW_PRODUCTION_DB_MIGRATIONS=false", "PRODUCTION_DB_MIGRATION_PLAN="].every((entry) => envExample.includes(entry)), ".env.example debe documentar Neon, el origen exacto y los opt-in de simulador y migraciones.");
