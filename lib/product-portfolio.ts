...
export function getLivingSpec(product: ProductDefinition, screen: string): LivingSpec {
  ...
  if (product.id === "kyc") return withGovernance(product.id, {
    ...
    flowAnalysis: [
      { step: "01 · Bienvenida", purpose: "Entregar valor y exploración segura antes de pedir identidad.", good: "Una promesa breve, CTA Explorar YOL1 y tres espacios que funcionan sin registro.", failure: "Hacer de Registrarme la única ruta o esconder la utilidad hasta después del acceso.", persist: "Sólo sesión anónima efímera y versión de copy; ningún dato de identidad." },
      { step: "02 · Elegir ruta y entender el gate", purpose: "Convertir una intención en una explicación concreta antes de pedir contacto.", good: "Dos rutas legibles con propósito, requisitos posibles, límites y salida para seguir explorando.", failure: "Pedir email/teléfono antes de que exista una intención o presentar transferencias, tarjeta o crédito como disponibles.", persist: "Intención local y capability_key candidata; no se crea pre-registro todavía." },
      { step: "03 · Elegir acceso", purpose: "Guardar una ruta concreta sin mezclar acceso con identidad financiera.", good: "Métodos visibles, explicación de uso del canal y campo único con validación comprensible.", failure: "Decir que Google/Facebook/Apple están activos si aún no hay IdP configurado, o pedir permisos sin contexto.", persist: "Después de token válido: cognito_sub + person_id / access_identity_id idempotentes; nunca el token en analytics." },
      { step: "04 · OTP y recuperación", purpose: "Comprobar control del canal y permitir volver sin fuga de información.", good: "Confirmación de canal, temporizador/reenvío, feedback directo y cambio de canal; respuesta neutral ante existente.", failure: "Código vencido sin salida, reintentos ilimitados, o confirmar que un contacto está registrado.", persist: "Challenge server-side con TTL, single-use, attempt bucket y session binding; no OTP/contacto crudo en logs analíticos." },
    ...
    technicalFlow: [
      { screen: "01 · Bienvenida", ui: "Propuesta de valor + Explorar YOL1 + tres espacios de descubrimiento", next: "Explorar → Elegir objetivo | Descubrimiento → Acompañante", services: ["App shell / feature flags", "Content config versionado"], command: "GET /onboarding/experience?entry_point=…", reads: ["copy_variant", "capability_catalog", "feature_flags"], writes: ["Ninguna escritura de identidad"], records: ["anonymous_session_id efímero"], events: ["onboarding_value_viewed", "onboarding_started"], failure: "Config inaccesible → fallback de copy estático; nunca bloquear la exploración.", acceptance: "El CTA principal entrega valor sin registro y las capacidades no aparecen como productos operativos." },
      { screen: "02 · Elegir objetivo", ui: "Dos rutas: entender finanzas o conocer la activación de Mi banco", next: "Ruta elegida → Explicar requisitos | Volver → Bienvenida", services: ["Capability catalog", "Policy/availability read model"], command: "GET /capabilities/{key}/preview", reads: ["capability matrix", "requirements_version", "availability_state"], writes: ["Sólo intención local hasta confirmar que se quiere guardar"], records: ["anonymous_session_id efímero"], events: ["material_action_selected", "requirements_explained"], failure: "Policy/partner desconocido → mostrar por validar y permitir volver; nunca habilitar por default.", acceptance: "La persona conoce propósito, límites y requisitos antes de entregar contacto." },
      { screen: "03 · Crear acceso", ui: "Google/Facebook/Apple candidatos + email/teléfono de demo", next: "Canal seleccionado → OTP | Cancelar → Requisitos", services: ["Cognito User Pool / IdP federado · candidato", "Identity BFF", "Rate-limit gateway"], command: "POST /access/challenges o OAuth /authorize", reads: ["provider_config", "anti_abuse_policy", "existing_identity_link"], writes: ["access_challenge_ref", "attempt_bucket", "correlation_id"], records: ["cognito_sub sólo después de token válido", "access_identity_id"], events: ["access_method_selected", "otp_requested"], failure: "IdP no disponible, formato inválido, rate limit o contacto existente → respuesta neutral y recuperación.", acceptance: "El contacto guarda una ruta ya elegida; no crea person_id desde cliente ni enumera cuentas." },
      { screen: "04 · Confirmar canal", ui: "OTP, reenvío, vencimiento, cambio de canal y ayuda", next: "Verificado → Resultado | Vencido/error → recuperar o cambiar canal", services: ["OTP challenge service", "Notification/SMS-email provider · pendiente", "Identity BFF", "Support router · candidato"], command: "POST /access/challenges/{ref}/verify", reads: ["challenge TTL", "attempt_count", "session_binding"], writes: ["channel_verified_at", "access_identity_id", "security_audit_event"], records: ["person_id resuelto idempotentemente en BFF", "preregistration_id ligado a la ruta elegida"], events: ["otp_verified", "otp_expired", "account_recovery_started", "support_route_started"], failure: "Código incorrecto/vencido o demasiados intentos → no se pierde intención y no se confirma existencia de cuenta.", acceptance: "Single-use, TTL, rate limit y binding server-side; OTP jamás llega a logs/analytics." },
      { screen: "05A · Entender finanzas", ui: "Cartola o consentimiento específico para datos", next: "Consentimiento aprobado → Acompañante | Cancelar → Tu espacio", services: ["Consent service", "Data connector adapter · pendiente", "Financial-data ingestion / normalizer · futuro"], command: "POST /consents/preview y, sólo si aprobado, POST /connections", reads: ["consent_scope", "provider availability", "data retention policy"], writes: ["consent_record", "consent_version", "connection_request_ref"], records: ["consent_id", "source_connection_ref opaco"], events: ["consent_preview_opened", "consent_granted", "consent_revoked"], failure: "Proveedor no disponible, usuario rechaza o fuente expira → explicar estado y volver sin datos parciales.", acceptance: "No pedir claves bancarias ni suponer SFA/conexión activa; consentimiento es específico y revocable." },
      { screen: "05B · Activar Mi banco", ui: "Checklist por capability: identidad, documento/biometría sólo si aplica", next: "Requisitos aprobados → handoff KYC | Sin ruta → estado por validar", services: ["Capability orchestration service", "KYC adapter · pendiente", "Risk/eligibility adapter · pendiente", "Customer Success router"], command: "POST /capability-requests/{id}/requirements", reads: ["capability policy", "partner contract status", "risk controls", "KYC state normalized"], writes: ["kyc_case_ref opaco", "requirement_status", "review_reason_code"], records: ["capability_request", "kyc_case_ref", "support_case_ref opcional"], events: ["kyc_requirements_viewed", "kyc_status_changed", "support_route_started"], failure: "Documento no legible, revisión, rechazo o estado proveedor desconocido → review, recaptura o ayuda; nunca approved por inferencia.", acceptance: "No capturar RUT/serie/biometría en el Lab; KYC no habilita dinero por sí solo." },
      { screen: "06 · Resultado / regreso", ui: "Qué quedó guardado, qué falta y cómo retomar", next: "Volver al Acompañante | Retomar checklist | Borrar demo", services: ["Onboarding state service", "Profile read model", "Support router"], command: "GET /onboarding/resume y DELETE /onboarding/demo-state", reads: ["resume_stage", "capability_request status", "support route state"], writes: ["resume preference", "audit deletion event"], records: ["snapshot demo no sensible / estado backend futuro"], events: ["preregistration_created", "onboarding_e2_answered", "preregistration_deleted"], failure: "Estado no recuperable o eliminación → informar con claridad y no mantener PII local.", acceptance: "La persona entiende que acceso/pre-registro no equivale a cuenta, KYC aprobado ni dinero habilitado." },
    ],
  });
