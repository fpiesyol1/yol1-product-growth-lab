...
export function getLivingSpec(product: ProductDefinition, screen: string): LivingSpec {
  ...
  if (product.id === "kyc") return withGovernance(product.id, {
    ...
    technicalFlow: [
      { screen: "01 · Bienvenida", ui: "Propuesta de valor + registro principal + tres espacios de descubrimiento", next: "Registrarme → Crear acceso | Descubrimiento → Acompañante", services: ["App shell / feature flags", "Content config versionado"], command: "GET /onboarding/experience?entry_point=…", reads: ["copy_variant", "capability_catalog", "feature_flags"], writes: ["Ninguna escritura de identidad"], records: ["anonymous_session_id efímero"], events: ["onboarding_value_viewed", "onboarding_started"], failure: "Config inaccesible → fallback de copy estático; nunca bloquear registro.", acceptance: "Un solo CTA dominante; las capacidades no aparecen como productos operativos." },
      { screen: "02 · Crear acceso", ui: "Google/Facebook/Apple candidatos + email/teléfono de demo", next: "Canal seleccionado → OTP | Cancelar → Bienvenida", services: ["Cognito User Pool / IdP federado · candidato", "Identity BFF", "Rate-limit gateway"], command: "POST /access/challenges o OAuth /authorize", reads: ["provider_config", "anti_abuse_policy", "existing_identity_link"], writes: ["access_challenge_ref", "attempt_bucket", "correlation_id"], records: ["cognito_sub sólo después de token válido", "access_identity_id"], events: ["access_method_selected", "otp_requested"], failure: "IdP no disponible, formato inválido, rate limit o contacto existente → respuesta neutral y recuperación.", acceptance: "No crear person_id desde cliente; no enumerar cuentas ni registrar contacto/token crudo." },
      { screen: "03 · Confirmar canal", ui: "OTP, reenvío, vencimiento, cambio de canal y ayuda", next: "Verificado → Tu espacio | Vencido/error → recuperar o cambiar canal", services: ["OTP challenge service", "Notification/SMS-email provider · pendiente", "Identity BFF", "Support router · candidato"], command: "POST /access/challenges/{ref}/verify", reads: ["challenge TTL", "attempt_count", "session_binding"], writes: ["channel_verified_at", "access_identity_id", "security_audit_event"], records: ["person_id resuelto idempotentemente en BFF", "preregistration_id sólo al guardar una ruta"], events: ["otp_verified", "otp_expired", "account_recovery_started", "support_route_started"], failure: "Código incorrecto/vencido o demasiados intentos → no se pierde intención y no se confirma existencia de cuenta.", acceptance: "Single-use, TTL, rate limit y binding server-side; OTP jamás llega a logs/analytics." },
      { screen: "04 · Tu espacio", ui: "Dos rutas: entender finanzas o activar Mi banco", next: "Ruta finanzas → Consentimiento | Ruta banco → Checklist de activación", services: ["Capability orchestration service", "Policy engine / feature flags", "Profile read model"], command: "POST /capability-requests", reads: ["person access state", "capability matrix", "policy_version", "partner availability"], writes: ["capability_request_id", "capability_key", "requirements_version", "availability_state"], records: ["capability_request", "preregistration"], events: ["material_action_selected", "capability_requested"], failure: "Policy/partner desconocido → availability=not_available o review; nunca habilitar por default.", acceptance: "La pantalla separa intención, acceso e identidad; no es una góndola de productos." },
